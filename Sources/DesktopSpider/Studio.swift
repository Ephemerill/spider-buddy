...
final class StudioController: NSObject, NSWindowDelegate, NSTextFieldDelegate, NSTextViewDelegate {
    ...
    private var grids: [(grid: OptionGrid, refresh: () -> Void)] = []
    ...
    private func build() {
        ...
        // Right: the part pickers.
        tabs = NSTabView()
        tabs.translatesAutoresizingMaskIntoConstraints = false
        tabs.widthAnchor.constraint(equalToConstant: 530).isActive = true
        tabs.heightAnchor.constraint(equalToConstant: 560).isActive = true
        tabs.addTabViewItem(tab("Body", buildBodyTab()))
        tabs.addTabViewItem(tab("Face", buildFaceTab()))
        tabs.addTabViewItem(tab("Legs", buildLegsTab()))
        tabs.addTabViewItem(tab("Colours", buildColourTab()))
        tabs.addTabViewItem(tab("Hats", buildHatTab()))
    ...
    private func partGrid<T: CaseIterable & Equatable>(
        ...
        grids.append((grid, { [weak self] in
            guard let self else { return }
            let cur = get()
            for (i, opt) in all.enumerated() {
                _ = self
                let cell = grid.cells[i]
                cell.isSelected = opt == cur
    ...
    private func buildBodyTab() -> NSView {
        ...
        grids.append((fuzzGrid, { [weak self] in
            guard let self else { return }
            for i in 0..<3 {
    ...
    private func buildColourTab() -> NSView {
        ...
        col.addArrangedSubview(partGrid(Coat.self, columns: 5, label: { $0.label },
                                        get: { self.design.look.coat }, set: { self.design.look.coat = $0 },
                                        preview: { var l = self.design.look; l.coat = $0; return l }))
        ...
        for (i, a) in accents.enumerated() {
            ...
            cell.label = a.label
            let c = a.rgb
            cell.swatch = NSColor(calibratedRed: c.r, green: c.g, blue: c.b, alpha: 1)
            cell.onPick = { [weak self] in self?.design.look.accent = a; self?.changed() }
        }
        grids.append((grid, { [weak self] in
            guard let self else { return }
            for (i, a) in accents.enumerated() { grid.cells[i].isSelected = self.design.look.accent == a }
        }))
    ...
    private func buildGaitTab() -> NSView {
        ...
        col.addArrangedSubview(slider("Leaping", low: "Stays put", high: "Window hopper",
                                      get: { self.design.gait.jumpiness }, set: { self.design.gait.jumpiness = $0 }))
        col.addArrangedSubview(slider("Silk", low: "Rarely", high: "Always dangling",
                                      get: { self.design.gait.webbiness }, set: { self.design.gait.webbiness = $0 }))
        return col
    }
